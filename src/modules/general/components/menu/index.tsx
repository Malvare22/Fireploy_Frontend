...
import { EstadoProyecto } from "@modules/proyectos/types/proyecto.estado";
import { Box, Button, Card, Grid2, Palette, Stack, Typography, useTheme } from "@mui/material";
...
const Menu:React.FC<MenuProps> = ({type= 'A'}) => {
    ...
    const proyectos = [proyecto1, proyecto2, proyecto1];
    ...
    const MenuBox = (
        <Grid2 container flexGrow={1} padding={4} spacing={6} rowSpacing={6} justifyContent={{xs: 'center', sm: 'normal'}}>
            {getMenuOptionsForUser().map((option, key) => <Grid2 size={{xs: 12, md: 6, xl: 4}} display='flex' justifyContent={'center'}><MenuCard key={key} color={option.color} icon={option.icon} label={option.label}/></Grid2>)}
        </Grid2>
   );
  return <Stack direction={{md:'row', sm: 'column'}} padding={2} justifyContent={'space-around'}>
    <TopProjects proyectos={proyectos}/>
...
const MenuCard: React.FC<MenuCardProps> = ({ color, icon, label }) => {
  return (
    ...
    >
      ...
      <Box sx={{ position: "absolute", bottom: 4, right: 4, '& svg': {
        fontSize: 48
      } }}>{icon}</Box>
    </AnimatedCard>
  );
  ...
  function ProjectItem(status: EstadoProyecto, label: string) {
    ...
    return (
      <Stack direction={"row"} spacing={1}>
        <Box>{ViewStatus}</Box>
        <Button variant="text" sx={{textTransform: 'none', textAlign: 'start', padding: 0}}><Typography>{label}</Typography></Button>
      </Stack>
    );
  }
  ...
  return (
    <Card sx={{ width: {md: 600, xs: '100%'}}}>
      <Stack spacing={1} margin={2}>
        ...
        {
            proyectos.map(proyecto => ProjectItem(proyecto.estado, proyecto.titulo))
        }
      </Stack>
    </Card>
  );
